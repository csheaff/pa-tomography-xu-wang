...
if __name__ == '__main__':
    ...
    xd = np.arange(-aperLen/2, aperLen/2+detPitch,detPitch)  #x coordinate of detector position

    fs = 20e6 #sampling frequency (Hz)
    ts = 1/fs #sampling period
    t = np.arange(0, 65e-6+ts, ts)  #time vector
    ...
    print(f'\nMean value of all recorded signals = {np.mean(sigs)}')
    print(f'Mean value of reconstructed volume = {np.mean(pfnorm)}')
